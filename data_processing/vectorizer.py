...
class VectorIndex:
    ...
    def _initialize_embed_model(self) -> Optional[HuggingFaceEmbedding]:
        ...
        '''
        logging.info('Initializing HuggingFace embedding model: %s', self.embedding_model_name)
        ...
        try:
            ...
            logging.info('Embedding model loaded. Detected dimension: %d', detected_dim)

            if detected_dim != self.embedding_dim:
                logging.warning('EMBEDDING_DIM %d != detected %d', self.embedding_dim, detected_dim)
    ...
    def _load_index(self) -> Optional[VectorStoreIndex]:
        ...
        try:
            ...
            logging.error('Failed to load index from %s: %s',
                           self.vector_store_path, e, exc_info=True)
            return None
    ...
    def _create_index(self, nodes: List[BaseNode]) -> Optional[VectorStoreIndex]:
        ...
        logging.info('Creating new vector index at %s from %d chunks',
                      self.vector_store_path, len(nodes))
        self.vector_store_path.mkdir(parents=True, exist_ok=True)

        try:
            quantizer = faiss.IndexFlatL2(self.embedding_dim)
            faiss_index = faiss.IndexIVFFlat(quantizer,
            self.embedding_dim, self.ivf_nlist, faiss.METRIC_L2)
            faiss_index.nprobe = self.ivf_nprobe
            logging.info('Initialized FAISS index (IndexIVFFlat) with dimension %d',
                          self.embedding_dim)

            texts = [node.get_content() for node in nodes]
            embeddings = np.array(self.embed_model.get_text_embedding_batch(texts,
            show_progress=True), dtype='float32')

            if not faiss_index.is_trained:
                logging.info('Training FAISS index (IndexIVFFlat) on %d vectors', len(embeddings))
            ...
            logging.error('Failed during index creation: %s', e, exc_info=True)
            return None

    def build_or_load_index(self, nodes: List[BaseNode],
                             force_reindex: bool = False) -> Optional[VectorStoreIndex]:
        '''
